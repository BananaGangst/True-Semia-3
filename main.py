...
version = "1.3" ##НАПИШИ ВЕРСИЮ Пример : "1.0"
...
async def echo_send(message : types.Message):
	if {i.lower().translate(str.maketrans('', '', string.punctuation)) for i in message.text.split(' ')}\
		...
		database.add_mute(user_id=message.reply_to_message.from_user.id, mute_time=600)
		await message.reply(f"❗Маты запрещены❗ @{message.reply_to_message.from_user.username} Замучен на {mute_time} минут!")
